...
(function($) {
    ...
    var times = null,
        ...
        scrollFn = {
            init: function() {
                scrollFn.sHeight = document.body.scrollTop + document.documentElement.scrollTop;
                scrollFn.aside = $(".right-container");
                scrollFn.offset = scrollFn.aside.height() - scrollFn.sHeight - $(window).height();
                scrollFn.offtop = scrollFn.sHeight - parseInt(scrollFn.aside.css("top"));
                scrollFn.offfixed = scrollFn.sHeight + 90;
            },
            down: function() {
                if (scrollFn.offset < 0) {
                    scrollFn.aside.css("top", -scrollFn.offset + "px")
                }
            },
            up: function() {
                if (scrollFn.offtop < -90) {
                    scrollFn.aside.css("top", scrollFn.offfixed + "px")
                }
            }
        }
    if (screen.width > 640) {
        $(".right-container").css("position","absolute");
